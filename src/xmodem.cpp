...
{
  volatile xmodem_status status = X_MODEM_OK;
  ...
  while (X_MODEM_OK == status)
  {
    ...
    case X_EOT:
      ...
      (void)uart_transmit_ch(X_ACK);
      (void)uart_transmit_str((uint8_t *)"Firmware updated!\r\n");
      return;
    /* Abort from host. */
    case X_CAN:
      status = X_ERROR;
...
static xmodem_status xmodem_handle_packet(uint8_t header)
{
  ...
  if ((X_MODEM_OK == status) && (false == x_first_packet_received))
  {
 ...
 * @return  status: X_ERROR in case of too many errors, X_MODEM_OK otherwise.
 */
static xmodem_status xmodem_error_handler(uint8_t *error_number, uint8_t max_error_number)
{
  xmodem_status status = X_MODEM_OK;
